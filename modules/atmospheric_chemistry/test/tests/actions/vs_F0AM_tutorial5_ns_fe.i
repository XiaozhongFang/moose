...
[Kernels]
  ...
  [y_momentum_space] type = INSMomentumLaplaceForm     variable = vel_y  u = vel_x  v = vel_y  pressure = p  component = 1  []
[]

[BCs]
  [x_no_slip]    type = DirichletBC          variable = vel_x  boundary = 'bottom right left'  value = 0.0  []
  [lid]          type = FunctionDirichletBC  variable = vel_x  boundary = 'top'                function = 'lid_function'  []
  [y_no_slip]    type = DirichletBC          variable = vel_y  boundary = 'bottom right top left'  value = 0.0  []
  [pressure_pin] type = DirichletBC          variable = p      boundary = 'pinned_node'         value = 0  []
[]

[Functions]
  [lid_function]  type = ParsedFunction  expression = '4*x*(1-x)'  []
[]
...
[Preconditioning]
  [SMP]  type = SMP  full = true  solve_type = 'NEWTON'  []
[]
