...
if [[ "$kind" == "module" ]]; then
    ...
    echo "      d. Add transitive module dependency chain if needed"
    echo "    2. Modify the moose/stork/Makefile.app and moose/stork/unit/Makefile.app files"
    echo "      a. Add the new module switch (e.g. NEW_MODULE := no) in alphabetical order"
    echo "    3. Modify the moose/modules/module_loader files"
    echo "      a. Add the new module to include/ModulesApp.h (include + registerAllObjects)"
    echo "      b. Add the new module to src/ModulesApp.C (include + registerApps + registerAll)"
    echo "    4. Modify the moose/scripts/sqa_stats.py file"
    echo "      a. Add a new compute requirements stats entry for the module"
    echo "    5. Modify the moose/modules/combined/src/base/CombinedApp.C file"
    ...
    echo "    6. Modify the moose/modules/doc/config.yml file"
    echo "       a. Add the module to the content listing (alphabetical)"
    echo "    7. Initialize module SQA (reach out to the MOOSE development team with questions)"
    ...
    echo "    8. Ensure that no stork files hang around before committing"
    echo "    9. Ensure that proper testing is performed for per-module tests (e.g. parallel, recover testing)"
    echo ""
